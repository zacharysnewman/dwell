...
// ---------------------------------------------------------------------------------------------
// Multi-player scenario (PPC Phase8Hardening / GoldenTrace).

namespace {

constexpr int kScenarioTicks = 600;
constexpr int kScenarioPlayers = 4;

// Floor, ramp, slab step, crawlspace roof, ladder to a ledge, pool, a moving and turning platform,
// and an explosion at tick 300.
void ScenarioWorld(PlayerTestWorld& w) {
  w.Floor(0, 24);
...
Input ScenarioInput(int tick, PlayerHandle player) {
...
std::vector<std::string> RecordTrace() {
  PlayerTestWorld w;
  ScenarioWorld(w);
